...
def run(args):
    ...
    cls_names_file = f"{args.save_data_path}/{args.data}_cls_names.pkl"
    ...
    torch.save(test_dl, f"{args.save_data_path}/{args.data}_test_dl_{args.batch_size}.pth")
    ...
    trainer = pl.Trainer(max_epochs = args.epochs, accelerator="gpu", devices = args.devices, strategy = "ddp", 
                         logger = wandb_logger,
                         fast_dev_run = True, # turn on for fast develeopment mode (debugging) 
                         callbacks = [EarlyStopping(monitor = "val_loss", mode = "min", patience = 3), ImagePredictionLogger(val_samples, cls_names),
                                      ModelCheckpoint(monitor = "val_loss", dirpath = args.save_model_path, filename = f"{args.data}_best")])
...
if __name__ == "__main__":
    ...
    parser.add_argument("-da", "--data", type = str, default = "genesis", help = "Data name")
    # parser.add_argument("-da", "--data", type = str, default = "hyundai", help = "Data name")
    # parser.add_argument("-da", "--data", type = str, default = "kia", help = "Data name")
    ...
    parser.add_argument("-sd", "--save_data_path", type = str, default = "saved_dlssss", help = "Path to the directory to save the dataset information")
